...
class UserProfileInfoForm(forms.ModelForm):
	 class Meta():
		 ...
		 widgets = {
			...
			'dob': forms.TextInput(attrs={'id':'dob','class': 'text_field' ,'placeholder': 'MM/DD/YYYY'}),
			
			}

class TransactionForm(forms.Form):
	Amount=forms.IntegerField(max_value=100000, label='Amount',widget=forms.NumberInput(attrs={'id':'Amount','class':'text_field'}))
	Action=forms.ChoiceField(choices=[('Lent','Lent'),('Borrowed','Borrowed')])
	Desc=forms.CharField(label='Description',max_length=500,widget=forms.TextInput(attrs={'id':'Desc','class':'text_field'}))

class UpdateProfileForm(forms.Form):
	name = forms.CharField(max_length=50 , widget=forms.TextInput(attrs={'id':'name','class':'text_field'}) )
	phone = forms.CharField(max_length=10 , widget=forms.TextInput(attrs={'id':'phone','class':'text_field'}) )
